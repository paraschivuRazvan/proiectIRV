...
public class DWGPhysicsCheck : MonoBehaviour {
	...
	void FixedUpdate(){
        if(this.GetComponent<Rigidbody>().IsSleeping()){ // Check if the rigidbody attached to this obeject is in a Sleep state
			if(!sleeping){ // If sleeping bool is false continue
				...
				if(timer > 2){ // If the timer is over 2 seconds continue
                    this.GetComponent<Rigidbody>().isKinematic = true; // Set the rigidbody to isKinematic, this sets the rigidbody to ignore physics
